...
function Convert-ToIsppStringLiteral {
...
function Invoke-BuildSetup {
    ...
    try {
        $scriptToCompile = $SetupScript
        $wrapperScript = ""

        if ($PreprocessorDefines.Count -gt 0) {
            $wrapperLeaf = [System.IO.Path]::GetFileNameWithoutExtension($SetupScript) + ".generated.iss"
            $wrapperScript = Join-Path $SetupDir $wrapperLeaf
            $wrapperLines = @(
                '; Generated by tools/build-setup.ps1. Do not edit.',
                ''
            )
            ...
            foreach ($define in ($PreprocessorDefines.GetEnumerator() | Sort-Object Key)) {
                ...
                $wrapperLines += "#define $($define.Key) $(Convert-ToIsppStringLiteral -Value $defineValue)"
            }

            $wrapperLines += @(
                '',
                "#include $(Convert-ToIsppStringLiteral -Value (Split-Path $SetupScript -Leaf))"
            )

            Set-Content -Path $wrapperScript -Value $wrapperLines -Encoding ascii
            $scriptToCompile = $wrapperScript
        }

        $isccArgs = @($scriptToCompile)
        ...
        if (-not [string]::IsNullOrWhiteSpace($wrapperScript) -and (Test-Path $wrapperScript)) {
