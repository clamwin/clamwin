...
import version
from cwd import CwServer
from asyncore import loop
import pyc

class MainWindow:
    MENU_OPEN_CLAM, MENU_UPDATE_DB, MENU_CHECK_UPDATE, MENU_CLAMWIN_WEB, MENU_CONFIGURE, MENU_SHOWSCANLOG, \
        MENU_SHOWUPDATELOG, MENU_RELOAD, MENU_EXIT, MENU_CONFIGURESCHEDULER,\
        MENU_TERMINATESCHEDULE, MENU_RUNSCHEDULE = range(1023, 1023 + 12)
    ...
    def _InitSchedulers(self, logon=False):
        ...
        if logon and self._config.Get('Updates', 'UpdateOnLogon'):
            ...
            scheduler = Scheduler.Scheduler('Once',
                            ...
                            ('ClamWin_Scheduler_Info', 'ClamWin_Update_Time'))
            scheduler.start()
    ...
    def OnTaskbarNotify(self, hwnd, msg, wparam, lparam):
        if lparam == win32con.WM_LBUTTONUP:
            ...
            win32gui.AppendMenu(menu, win32con.MF_SEPARATOR, 0, '')
            win32gui.AppendMenu(menu, win32con.MF_STRING, self.MENU_RELOAD, 'Reload CWD Conf and Virus DB')
            win32gui.AppendMenu(menu, win32con.MF_SEPARATOR, 0, '')
...
class CWD(threading.Thread, CwServer):
    def __init__(self):
        print 'CWD: Starting...'
        threading.Thread.__init__(self)
        CwServer.__init__(self)
        self.handler.do_QUIT = self.do_QUIT
        self.handler.do_RELOAD = self.do_RELOAD
        print 'CWD: Startup Done'

    def do_QUIT(self):
        print 'CWD: Quit command disabled'

    def do_RELOAD(self):
        print 'CWD: Reloading configuration'
        self.config.engage()
        print 'CWD: Reloading virus DB'
        pyc.checkAndLoadDB()
        print 'CWD: Done Reloading'
